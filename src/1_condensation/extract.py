...
class Extract:
    """A class used to extract anomalous fragments from a WAV file"""

    def __init__(self, wav_file_path=None):
        """
        Initialization of WAV file path. If no path is provided
        work with an empty signal.
        Args:
    ...
    def set_signal(self, signal, sr):
        """
        Instead of reading a WAV file, you can also set a signal
        manually.
        Args:
            signal (numpy.array): a signal
            sr (int): signal rate of signal
        """
        self.signal, self.sr = signal, sr

    # + operator add signals if sr is identical
    def __add__(self, other, dtype=np.int16):
        """
        Adds 2 signals provided the signal rates of both signals are
        identical.
        Args:
            other (Extract): another extract object containing a signal
            dtype (np.DataType): numpy Data type to ensure both signals 
                are formatted with the same Data type
        Returns:
            Extract: Extract containing the concatenation of both signals
        """
        if self.sr is not None and self.sr == other.sr:
            new_signal = np.concatenate((self.signal, other.signal))
            new_extraction = Extract()
            new_extraction.set_signal(new_signal.astype(dtype), self.sr)
    ...
    # min/max_threshold_db: loudness of vocalisations I am looking for, above
    # ref db's
    def detect_vocalisations(
        self, freqs=None, n_fft=2048, win_length=2048,
        hop_length=512, min_threshold_db=30, max_threshold_db=None,
        threshold_pattern=0.1, ref=np.median, min_voc_duration=0.5,
        padding=0.3, ignore_voc=0.0, use_cached_stft=True
    ):

        if (use_cached_stft is False or
                (use_cached_stft is True and self.stft is None)):
            # perform stft
            self.stft = stft(
                self.signal,
                fs=self.sr,
        ...
        for (low, high) in freqs:
            ...
            if max_threshold_db is None:
                ...
                max_dbs[(max_dbs < min_threshold_db) |
                        (max_dbs > max_threshold_db)] = 0
            ...
            dbs_suppressed = self._suppress_noise_patterns(
                dbs_filtered,
                window=5
            )
    ...
    def _time_2_sample_unit(self, t, time_type):
        if time_type == 'start':
            return int(np.floor(t * self.sr))
        else:
            return int(np.ceil(t * self.sr))
    ...
    def _compute_window(
        self, dbs, x_start, y_start, distribs, alpha=3, window_x=4,
        window_y=4
    ):
    ...
    def _suppress_noise_patterns(self, dbs, window=4):
        ...
        for i_x in range(dbs.shape[1] - window):
            for i_y in range(dbs.shape[0] - window):
                ...
                # leaving us with only positive differences. So if things are
                # louder in specific time/frequency window in comparison with
                # the cum power distribution of the entire time interval we
                # get a possitive number back.
...
if __name__ == '__main__':
    path = ''
    extracted = Extract()
    extracted.set_signal(np.array([1, 1, 1, 1, 1, 1, 1]), 2)
    print(len(extracted))




# f = Filter('/Users/casperkaandorp/Downloads/mandrills/20191216_135958.WAV')
# vocals = f.detect_vocalisations(freqs=[(10, 400)], threshold_db=23,
# threshold_pattern=0.1, ignore_voc=0.09)
