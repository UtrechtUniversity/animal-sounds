...
class ProcessRaven:
    ...
    def split_multifile_annotations(df, filelist, wav_path):
        ...
        DataFrame
            Cleaned dataframe
        """
        row_i = list(df[(df['file']
                         != df['end_file'])].index)

        for i in row_i:
            df_temp = pd.DataFrame([[df.loc[i, 'file'],
                                     df.loc[i, 'file'],
                                     df.loc[i, 'start_time'],
                                     df.loc[i, 'filelength'],
                                     df.loc[i, 'filelength']],
            ...
            if (filelist.index(df.loc[i, 'end_file']) -
                filelist.index(df.loc[i, 'file'])) > 1:

                for j in range(filelist.index(df.loc[i, 'file'])
                               + 1, filelist.index(df.loc[i, 'end_file'])):
                    print(j)
    ...
    def process_raven(self, species, min_sig_len):
        ...
        df['file'] = df['file'].str.replace('.*\\\\',
                                                        '', regex=True)
        df['end_file'] = df['end_file'].str.replace('.*\\\\',
                                                    '', regex=True)
