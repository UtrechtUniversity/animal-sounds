...
if __name__ == '__main__':
    ...
    avg_bg_frames = int(sum([x['frames'] for x in background_frames]) / len(background_frames))
    ...
    for i, item in enumerate(primate_frames):
        ...
        path = item['path']
        frames = item['frames']
        item['voc'] = AudioSegment.from_file(path, format='wav', frame_rate=args['sample_rate'])
        item['postfix'] = ''
    ...
    for i, p in enumerate(primate_set):
        ...
        match['voc'] = AudioSegment.from_file(match['path'], format='wav', frame_rate=args['sample_rate'])
