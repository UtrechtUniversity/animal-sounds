...
def fft2barkmx(fft_length, fs, nfilts=0, band_width=1, min_freq=0, max_freq=0):
    ...
    binbarks = hz2bark(
        np.multiply(
            np.arange(0, np.add(np.divide(fft_length, 2), 1)), np.divide(fs, fft_length)
        )
    )
...
def lpc2cep(a, nout=0):
    ...
    for n in range(1, nout):
        ...
        for m in range(1, n):
            sum = np.add(
                sum, np.multiply(np.multiply((n - m), norm_a[m, :]), cep[(n - m), :])
            )
...
def melfcc(
    ...
    pspectrum, logE = powspec(
        x, fs=fs, window_time=window_time, hop_time=hop_time, dither=dither
    )
...
def fft2melmx(
    ...
    binfrqs = mel2hz(
        np.add(
            min_mel,
            np.multiply(np.arange(0, nfilts + 2), (max_mel - min_mel) / (nfilts + 1)),
        ),
        htk,
    )
    ...
    for i in range(int(nfilts)):
        ...
        fs_tmp = np.add(
            fs_tmp[1], np.multiply(band_width, np.subtract(fs_tmp, fs_tmp[1]))
        )
        loslope = np.divide(
            np.subtract(fftfrqs, fs_tmp[0]), np.subtract(fs_tmp[1], fs_tmp[0])
        )
        hislope = np.divide(
...
def audspec(
    ...
    if fbtype == "bark":
        ...
        wts = fft2melmx(
            nfft, fs, nfilts, band_width, min_freq, max_freq, htk=True, constamp=True
        )
    elif fbtype == "fcmel":
        wts = fft2melmx(
    ...
    if dcttype == 2 or dcttype == 3:
        for i in range(ncep):
            dctm[i, :] = np.multiply(
                np.cos(
                    np.multiply(
                        np.divide(
                            np.multiply(i, np.arange(1, 2 * nrow, 2)), (2 * nrow)
                        ),
                        np.pi,
                    )
                ),
        ...
        for i in range(ncep):
            dctm[i, :] = np.multiply(
                np.cos(
                    np.multiply(
                        np.divide(np.multiply(i, np.arange(1, nrow + 1)), (nrow + 1)),
                        np.pi,
                    )
                ),
                2,
            ...
            dctm[i, int(nrow - 1)] = np.multiply(
                dctm[i, int(nrow - 1)], np.power(-1, i)
            )
...
def lpc2spec(lpcas, nout=17, FMout=False):
    ...
    features = np.divide(
        np.power(np.divide(1, np.abs(np.matmul(zz, aa))), 2), np.tile(gg, (nout, 1))
    )
    ...
    if FMout:
        for c in range(cols):
            ...
            zz = np.exp(
                np.multiply(
                    1j, np.matmul(ff, np.array(np.arange(0, aaa.shape[0]), ndmin=2))
                )
            )
            mags = np.sqrt(
                np.divide(
                    np.power(
                        np.divide(1, np.abs(np.matmul(zz, np.array(aaa, ndmin=2).T))), 2
                    ),
                    gg[c],
                )
            )
    ...
    if dcttype == 2 or dcttype == 3:
        for i in range(ncep):
            dctm[i, :] = np.multiply(
                np.cos(
                    np.multiply(
                        np.divide(
                            np.multiply(i, np.arange(1, 2 * nfreq, 2)), (2 * nfreq)
                        ),
                        np.pi,
                    )
                ),
        ...
        for i in range(ncep):
            idctm[:, i] = np.multiply(
                np.cos(
                    np.multiply(
                        np.divide(
                            np.multiply(i, np.arange(1, nfreq + 1).T), (nfreq + 1)
                        ),
                        np.pi,
                    )
                ),
        ...
        for i in range(ncep):
            idctm[:, i] = np.multiply(
                np.cos(
                    np.multiply(
                        np.divide(np.multiply(i, np.arange(0, nfreq).T), (nfreq - 1)),
                        np.pi,
                    )
                ),
                2,
...
def invpowspec(y, fs, win_time, hop_time, excit=[]):
    ...
    tmp = librosa.istft(
        y, hop_length=steppts, win_length=winpts, window="hann", center=False
    )
    xlen = len(tmp)
    ...
    x = librosa.istft(
        R, hop_length=steppts, win_length=winpts, window="hann", center=False
    )
...
def invaudspec(
    ...
    itws = np.divide(
        wts.T,
        np.tile(
            np.maximum(np.divide(np.mean(np.diag(ww)), 100), np.sum(ww, axis=0)),
            (nfilts, 1),
        ).T,
    )
    if sumpower:
...
# Zero Crossing Rate
def ZCR(X):
    return np.mean(np.abs(np.diff(X > 0, axis=1)), axis=1)


# reference
# https://labrosa.ee.columbia.edu/matlab/rastamat/
# https://raw.githubusercontent.com/mystlee/rasta_py/master/rasta.py
